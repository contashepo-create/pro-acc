...
export default function InvoicesPage() {
  ...
  const [form, setForm] = useState<any>({
    client_id: '', project_id: '', date: new Date().toISOString().split('T')[0], due_date: '', notes: '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/invoices', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId: form.client_id, projectId: form.project_id || null, date: form.date, dueDate: form.due_date || form.date,
          items: form.items.map(i => ({ description: i.description, quantity: i.quantity, unitPrice: i.unitPrice, total: i.total })),
          subtotal, vatRate, vatAmount, total, notes: form.notes }) });
      const json = await res.json();
      if (json.success) { setShowModal(false); setForm({ client_id: '', project_id: '', date: new Date().toISOString().split('T')[0], due_date: '', notes: '',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [invRes, cliRes, projRes] = await Promise.all([fetch('/api/invoices'), fetch('/api/clients'), fetch('/api/projects')]);
        const [invJson, cliJson, projJson] = await Promise.all([invRes.json(), cliRes.json(), projRes.json()]);
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, { variant: 'success' | 'warning' | 'danger' | 'info'; label: string }> = {
      unpaid: { variant: 'warning', label: 'غير مدفوعة' }, partial: { variant: 'info', label: 'مدفوعة جزئياً' },
      paid: { variant: 'success', label: 'مدفوعة' }, cancelled: { variant: 'danger', label: 'ملغاة' },
    };
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? <EmptyState title="لا توجد فواتير" description="أضف فاتورة جديدة" actionLabel="إضافة فاتورة" onAction={() => setShowModal(true)} /> : <DataTable columns={columns} data={filtered} searchable searchKeys={['contact_name', 'number']} />}
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="إضافة فاتورة جديدة" size="xl" footer={<div className="flex gap-2"><Button variant="ghost" onClick={() => setShowModal(false)}>إلغاء</Button><Button onClick={handleSave} disabled={saving}>{saving ? "جاري الحفظ..." : "حفظ"}</Button></div>}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Select label="العميل" value={form.client_id} onChange={(v) => setForm({...form, client_id: v})} options={[{ value: '', label: 'اختر عميلاً' }, ...clients.map(c => ({ value: c.id, label: c.name }))]} className="col-span-2" />
            <Select label="المشروع (اختياري)" value={form.project_id} onChange={(v) => setForm({...form, project_id: v})} options={[{ value: '', label: 'بدون مشروع' }, ...projects.map(p => ({ value: p.id, label: p.name }))]} className="col-span-2" />
          </div>
