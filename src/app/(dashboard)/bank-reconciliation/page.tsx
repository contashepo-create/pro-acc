    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="تسوية بنكية جديدة" size="lg" footer={<div className="flex items-center gap-2"><Button variant="ghost" onClick={() => setShowModal(false)}>إلغاء</Button><Button>حفظ التسوية</Button></div>}>
        <div className="space-y-4">
          ...
          <Input label="تاريخ التسوية" type="date" value={form.تاريخ_التسوية} onChange={(e) => setForm({...form, تاريخ_التسوية: e.target.value})} />
          <Input label="الرصيد الختامي حسب كشف الحساب" type="number" value={form.الرصيد_الختامي_حسب_كشف_الحساب} onChange={(e) => setForm({...form, الرصيد_الختامي_حسب_كشف_الحساب: e.target.value})} />
