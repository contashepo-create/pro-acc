...
export default function JournalPage() {
  ...
  const [form, setForm] = useState<any>({
    date: new Date().toISOString().split('T')[0], type: 'general', description: '',
    lines: [{ accountCode: '', debit: 0, credit: 0, description: '' }, { accountCode: '', debit: 0, credit: 0, description: '' }] as JournalLine[],
  });
  ...
  const removeLine = (index: number) => { if (form.lines.length <= 2) return; setForm({ ...form, lines: form.lines.filter((_: any, i: number) => i !== index) }); };
  const updateLine = (index: number, field: keyof JournalLine, value: any) => {
    const newLines = [...form.lines]; newLines[index] = { ...newLines[index], [field]: value }; setForm({ ...form, lines: newLines });
  };
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditingEntry(null); }} title={editingEntry ? `تعديل قيد رقم #${editingEntry.number}` : 'إضافة قيد محاسبي'} size="xl" footer={<div className="flex gap-2"><Button variant="ghost" onClick={() => { setShowModal(false); setEditingEntry(null); }}>إلغاء</Button><Button onClick={handleSave} disabled={saving}>{saving ? "جاري الحفظ..." : "حفظ"}</Button></div>}>
        <div className="space-y-4">
          ...
          <Textarea label="البيان" value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} placeholder="شرح القيد المحاسبي" />
          <div className="border border-border rounded-lg overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-bg-secondary"><tr><th className="p-2 text-right">الحساب</th><th className="p-2 text-right w-28">مدين</th><th className="p-2 text-right w-28">دائن</th><th className="p-2 text-right">البيان</th><th className="p-2 w-10"></th></tr></thead>
              <tbody>{form.lines.map((line, i) => (<tr key={i} className="border-t border-border">
                <td className="p-2"><Select value={line.accountCode} onChange={(v) => updateLine(i, 'accountCode', v)} options={[{ value: '', label: 'اختر حساباً' }, ...accounts.filter(a => !a.parent_id || a.children?.length === 0).map(a => ({ value: a.code, label: `${a.code} - ${a.name}` }))]} /></td>
                ...
                <td className="p-2"><Input placeholder="شرح" value={line.description} onChange={(e) => updateLine(i, 'description', e.target.value)} /></td>
                <td className="p-2">{form.lines.length > 2 && <Button variant="ghost" size="sm" onClick={() => removeLine(i)}><Trash2 size={14} className="text-danger" /></Button>}</td>
              </tr>))}</tbody>
            </table>
          </div>
          <div className="flex justify-between items-center">
            ...
            <div className="text-left space-y-1 text-sm">
              ...
              <div className={`text-lg font-bold ${isBalanced ? 'text-success' : 'text-danger'}`}>{isBalanced ? '✅ متوازن' : `❌ الفرق: ${formatCurrency(Math.abs(totalDebit - totalCredit))}`}</div>
            </div>
          </div>
      ...
      <Modal isOpen={!!showDetail} onClose={() => setShowDetail(null)} title={`قيد رقم #${showDetail?.number || ''}`} size="lg">
        {showDetail && (<div className="space-y-4">
          ...
          <div className="border border-border rounded-lg overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-bg-secondary"><tr><th className="p-2 text-right">الحساب</th><th className="p-2 text-right">مدين</th><th className="p-2 text-right">دائن</th><th className="p-2 text-right">البيان</th></tr></thead>
              <tbody>{(showDetail.lines || []).map((line: any, i: number) => (<tr key={i} className="border-t border-border">
              ...
              <tfoot className="bg-bg-secondary font-bold"><tr>
