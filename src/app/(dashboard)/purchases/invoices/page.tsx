...
export default function PurchaseInvoicesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [invJson, supJson] = await Promise.all([
          invRes.json(),
          supRes.json(),
        ]);
        if (invJson.success) {
          ...
          setError(invJson.message || 'فشل تحميل البيانات');
        }
        if (supJson.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="إضافة فاتورة مشتريات" size="xl" footer={
