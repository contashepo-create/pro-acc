...
export default function PurchaseInvoicesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/purchases/invoices', { method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [invRes, supRes] = await Promise.all([fetch('/api/purchases/invoices'), fetch('/api/contacts?type=supplier')]);
  ...
  const columns = [
    ...
    { key: 'paid_amount', label: 'المدفوع', sortable: true, render: (row: any) => formatCurrency(row.paid_amount || 0) },
    { key: 'status', label: 'الحالة', render: (row: any) => row.status === 'paid' ? 'مدفوعة' : 'غير مدفوعة' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {invoices.length === 0 ? <EmptyState title="لا توجد فواتير مشتريات" description="أضف فاتورة مشتريات جديدة" actionLabel="إضافة فاتورة" onAction={() => setShowModal(true)} /> : <DataTable columns={columns} data={invoices} searchable searchKeys={['supplier_name', 'invoice_number']} />}
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="إضافة فاتورة مشتريات" size="xl" footer={<div className="flex gap-2"><Button variant="ghost" onClick={() => setShowModal(false)}>إلغاء</Button><Button onClick={handleSave} disabled={saving}>{saving ? "جاري الحفظ..." : "حفظ"}</Button></div>}>
