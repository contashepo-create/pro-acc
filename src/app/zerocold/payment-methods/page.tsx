    ...
    <div className="min-h-screen bg-bg-primary text-text-primary">
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[0.7rem] text-text-muted">تحكم في طرق الدفع المتاحة للعملاء (انستا باي، أورنج كاش، بنكي)</p>
            </div>
          </div>
        ...
        <div className="grid gap-4">
          {methods.length === 0 ? (
            <div className="bg-bg-card border border-border rounded-2xl p-12 text-center text-amber-600/50">لا توجد طرق دفع - أضف انستا باي، أورنج كاش، تحويل بنكي</div>
          ) : methods.map((m) => (
            <div key={m.id} className="bg-bg-card border border-border rounded-2xl p-5">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => toggleActive(m)} className={`w-12 h-6 rounded-full transition-colors relative ${m.is_active ? 'bg-green-600' : 'bg-bg-secondary'}`}>
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <div className="bg-bg-card border border-border rounded-2xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-3">
                <input placeholder="الكود (مثلاً: instapay)" value={form.code} onChange={(e) => setForm({...form, code: e.target.value})} className="w-full px-4 py-2.5 bg-bg-secondary border border-border rounded-xl text-sm" />
