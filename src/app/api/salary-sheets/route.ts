...
export async function POST(req: NextRequest) {
  try {
    ...
    const effectiveDate = String(date ?? new Date().toISOString().slice(0, 10));
    ...
    const normalized: { employee_id: unknown; basic_salary: number; allowances: number; deductions: number; project_id: unknown }[] = (items || []).map((item: Row) => ({
    ...
    if (normalized.some((item) => !item.employee_id || ![item.basic_salary,item.allowances,item.deductions].every((value) => Number.isFinite(value) && value>=0)
      || item.basic_salary+item.allowances-item.deductions<0)
      || new Set(normalized.map((item) => item.employee_id)).size!==normalized.length) return error('أحد بنود كشف الرواتب غير صالح أو مكرر');
    if (normalized.some((item: Row) => item.project_id && (typeof item.project_id !== 'string' || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(item.project_id)))) {
      return error('معرّف المشروع المرتبط بأحد البنود غير صالح');
    }
    if (normalized.length) {
      const employeeIds = normalized.map((item: Row) => item.employee_id);
      const projectIds = normalized.map((item: Row) => item.project_id).filter(Boolean);
