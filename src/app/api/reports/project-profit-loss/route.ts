...
import type { SupabaseLike, Row } from '@/lib/types';

const number = (value: unknown) => Number(value) || 0;

/** Fetch allocated overhead (indirect costs) per project for the period. */
async function fetchOverheadMap(s: SupabaseLike, companyId: string, projectIds: string[], from: string | null, to: string | null) {
  ...
  for (const row of (data ?? []) as Row[]) map.set(String(row.project_id), number(row.allocated_overhead));
  return map;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (projectId) {
      ...
      const billing = ((billingResult.data ?? []) as Row[])[0] || ({} as Row);
      ...
      for (const account of journal.accounts) {
        ...
        total += net;
        costs[classifyProjectCost(String(account.code))] += net;
      }
      ...
      const contractValue = number((project as Row).contract_value);
      return success({
        project: {
          id: String(project.id), name: String(project.name), client_name: (project as Row).contacts ? String(((project as Row).contacts as Row).name) || null : null,
          contract_value: contractValue, status: project.status,
          start_date: (project as Row).start_date, end_date: (project as Row).end_date,
        },
        financials: {
    ...
    const result = ((projects ?? []) as Row[]).map((project: Row) => {
      ...
      const costs = journalMap[String(project.project_id)]?.expenses || 0;
      const directProfit = revenue - costs;
      const allocatedOverhead = overheadMap.get(String(project.project_id)) || 0;
      const profit = directProfit - allocatedOverhead;
      return {
        id: project.project_id, name: project.name, client_name: project.client_name || null,
        contract_value: contractValue, billed_amount: number((billingMap.get(String(project.project_id)) as Row)?.net_billed),
    ...
    const totals = result.reduce((acc: Record<string, number>, project) => ({
      total_contract_value: acc.total_contract_value + number(project.contract_value),
      total_billed: acc.total_billed + number(project.billed_amount),
      total_revenue: acc.total_revenue + number(project.revenue),
      total_costs: acc.total_costs + number(project.costs),
      total_overhead: acc.total_overhead + number(project.allocated_overhead),
      total_profit: acc.total_profit + number(project.profit),
