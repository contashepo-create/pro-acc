...
/** GET /api/suppliers/[id]/statement?from=&to=&page=&pageSize= */
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const auth = await requireModulePermission(req, 'suppliers', 'read');
    const { id } = await params;
    if (!UUID_RE.test(id)) return error('معرّف المورد غير صالح');
    const s = sb();
    ...
    const { data: supplier, error: supplierError } = await s.from('contacts')
      .select('id, name, type, phone, email, address, tax_number, commercial_registration')
      .eq('id', id).eq('company_id', auth.companyId).in('type', ['supplier', 'both']).maybeSingle();
    if (supplierError) throw supplierError;
    if (!supplier) return error('المورد غير موجود', 404);

    const offset = (page - 1) * pageSize;
    const [summaryResult, linesResult, purchasesResult, disbursementsResult] = await Promise.all([
      ...
      (() => {
        let query = s.from('purchase_invoices').select('id, number, date, total, paid_amount, status', { count: 'exact' })
          .eq('supplier_id', id).eq('company_id', auth.companyId);
        if (from) query = query.gte('date', from);
        if (to) query = query.lte('date', to);
        return query.order('date', { ascending: true }).range(0, 499);
      })(),
      (() => {
        let query = s.from('voucher_disbursements').select('id, number, date, amount, status, reason', { count: 'exact' })
    ...
    return success({
      ...
      purchase_invoices: purchasesResult.data || [],
      disbursements: disbursementsResult.data || [],
      supportingDocumentsTruncated: (purchasesResult.count || 0) > 500 || (disbursementsResult.count || 0) > 500,
