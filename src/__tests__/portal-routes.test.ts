...
jest.mock('@/lib/email', () => ({ sendEmail: (...args: any[]) => sendEmailMock(...args) }));
...
describe('portal/auth POST', () => {
  test('returns the generic message when no matching contact exists', async () => {
    const res = await portalAuthPOST({ url: 'http://localhost/api/portal/auth', nextUrl: new URL('http://localhost/api/portal/auth'), headers: { get: () => null }, json: async () => ({ email: EMAIL }) } as any);
  ...
  test('rejects an invalid email shape', async () => {
    const res = await portalAuthPOST({ url: 'http://localhost/api/portal/auth', nextUrl: new URL('http://localhost/api/portal/auth'), headers: { get: () => null }, json: async () => ({ email: 'not-an-email' }) } as any);
...
describe('portal/invoices GET', () => {
  ...
  test('rejects a missing/invalid portal token', async () => {
    const res = await portalInvoicesGET({ url: 'http://localhost/api/portal/invoices', headers: { get: () => null } } as any);
    expect(res.status).toBe(401);
  });

  test('rejects when the contact no longer matches', async () => {
    const res = await portalInvoicesGET({ url: 'http://localhost/api/portal/invoices', headers: { get: (k: string) => k === 'x-portal-token' ? portalToken : null } } as any);
    expect(res.status).toBe(401);
  });
