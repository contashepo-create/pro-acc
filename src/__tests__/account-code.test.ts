...
describe('account codes — order & RTL aliases', () => {
  ...
  test('handles empty lookup and every legacy child-code ordering', () => {
    ...
    expect(nextChildAccountCode('1110', ['4-1110', '11100002'])).toBe('1110-0005');
  ...
  test('searches aliases in order and returns null when no account matches', async () => {
    ...
    const supabase = wrapSupabase({ from: () => {
      const api: TestBuilder = { select: () => api, eq: (_field: string, value: string) => { if (_field === 'code') calls.push(value); return api; }, maybeSingle: async () => ({ data: calls.at(-1) === '1110-0001' ? { id: 'a1', code: '1110-0001' } : null, error: null }) };
      return api;
    } });
    await expect(findAccountByCode(supabase, 'c1', '0001-1110')).resolves.toMatchObject({ id: 'a1' });
    await expect(findAccountByCode(wrapSupabase({ from: () => { const api: TestBuilder = { select: () => api, eq: () => api, maybeSingle: async () => ({ data: null, error: null }) }; return api; } }), 'c1', '9999')).resolves.toBeNull();
  });
