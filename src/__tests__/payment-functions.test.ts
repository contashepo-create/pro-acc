...
describe('Moyasar payment gateway functions', () => {
  test('initializes payment in halalas with invoice/customer metadata', async () => {
    global.fetch = jest.fn(async () => new Response(JSON.stringify(payment), { status: 200 })) as any;
  ...
  test('falls back to payment API URL and reports init failures', async () => {
    global.fetch = jest.fn(async () => new Response(JSON.stringify({ ...payment, invoice_url: undefined }), { status: 200 })) as any;
    ...
    global.fetch = jest.fn(async () => new Response('bad card', { status: 422 })) as any;
    await expect(initPayment({ amount: 1, description: 'x', callbackUrl: 'https://a', invoiceId: 'i', customerName: 'n', customerEmail: 'e' }))
  ...
  test('gets payment status and rejects failed lookups', async () => {
    ...
    global.fetch = jest.fn(async () => new Response('', { status: 404 })) as any;
  ...
  test('refunds full/partial amounts and exposes gateway failures', async () => {
    global.fetch = jest.fn(async () => new Response(JSON.stringify({ ...payment, status: 'refunded' }), { status: 200 })) as any;
    ...
    global.fetch = jest.fn(async () => new Response('not captured', { status: 409 })) as any;
