...
describe('purchase invoice atomic boundary', () => {
  ...
  test('metadata update cannot manually manufacture a paid state', async () => {
    const response = await invoicePUT(request({ status: 'paid' }, 'PUT'), params('pi-1'));
    expect(response.status).toBe(409);
    expect(mockDb.rpcCalls).toHaveLength(0);
  });

  test('notes use metadata RPC while cancellation and DELETE use the reversal lifecycle', async () => {
    expect((await invoicePUT(request({ notes: 'new' }, 'PUT'), params('pi-1'))).status).toBe(200);
    expect(rpc('update_purchase_invoice_metadata')!.params).toMatchObject({ p_company_id: C1, p_invoice_id: 'pi-1', p_user_id: USER });

    mockDb.rpcCalls.length = 0;
    expect((await invoicePUT(request({ status: 'cancelled', notes: 'خطأ' }, 'PUT'), params('pi-1'))).status).toBe(200);
    expect(rpc('cancel_purchase_invoice_atomic')!.params).toMatchObject({ p_company_id: C1, p_invoice_id: 'pi-1', p_notes: 'خطأ' });

    mockDb.rpcCalls.length = 0;
    expect((await invoiceDELETE(request(undefined, 'DELETE'), params('pi-1'))).status).toBe(200);
    expect(rpc('cancel_purchase_invoice_atomic')!.params).toMatchObject({ p_company_id: C1, p_invoice_id: 'pi-1', p_user_id: USER });
  });
});
...
describe('purchase order atomic boundary', () => {
  ...
  test('update is tenant-scoped in the RPC call', async () => {
    const response = await orderPUT(request({ notes: 'updated' }, 'PUT'), params('po-1'));
    expect(response.status).toBe(200);
    expect(rpc('update_purchase_order_atomic')!.params).toMatchObject({
      p_company_id: C1, p_order_id: 'po-1', p_notes: 'updated', p_user_id: USER,
    });
  });

  test('negative receive quantity is rejected before stock can be touched', async () => {
    const response = await orderPATCH(request({ quantities: { 'line-1': -1 } }, 'PATCH'), params('po-1'));
    expect(response.status).toBe(400);
    expect(mockDb.rpcCalls).toHaveLength(0);
  ...
  test('receipt and cancellation carry trusted company/user into locked RPCs', async () => {
    expect((await orderPATCH(request({ quantities: { 'line-1': 2 }, date: '2026-08-02' }, 'PATCH'), params('po-1'))).status).toBe(200);
    expect(rpc('receive_purchase_order_atomic')!.params).toMatchObject({
      p_company_id: C1, p_order_id: 'po-1', p_quantities: { 'line-1': 2 }, p_user_id: USER,
    });
    mockDb.rpcCalls.length = 0;
    expect((await orderDELETE(request(undefined, 'DELETE'), params('po-1'))).status).toBe(200);
    expect(rpc('cancel_purchase_order_atomic')!.params).toMatchObject({ p_company_id: C1, p_order_id: 'po-1', p_user_id: USER });
  });
});
...
describe('purchase reads remain tenant scoped and batched', () => {
  test('foreign invoice/order ids return 404 because both parent reads filter company_id', async () => {
    mockDb = makeDb({ ...baseDb(),
      purchase_invoices: [{ id: 'pi-x', company_id: 'company-2' }],
      purchase_orders: [{ id: 'po-x', company_id: 'company-2' }],
    });
    expect((await invoiceGET(request(undefined, 'GET'), params('pi-x'))).status).toBe(404);
    expect((await orderGET(request(undefined, 'GET'), params('po-x'))).status).toBe(404);
  ...
  test('invoice supplier filter and page-level item/payment queries are preserved', async () => {
    mockDb = makeDb({ ...baseDb(),
      purchase_invoices: [{ id: 'pi-1', company_id: C1, supplier_id: SUPPLIER, date: '2026-08-01', paid_amount: 0 }],
      purchase_invoice_items: [{ id: 'line-1', company_id: C1, purchase_invoice_id: 'pi-1' }],
      disbursement_invoice_items: [{ company_id: C1, purchase_invoice_id: 'pi-1', amount: 80 }],
    });
    const response = await invoicesGET(request(undefined, 'GET', `http://localhost/api/purchases/invoices?supplierId=${SUPPLIER}`));
    const json = await response.json();
    expect(json.data.invoices[0]).toMatchObject({ id: 'pi-1', paid_amount: 80 });
  ...
  test('orders load children in one page-level query', async () => {
    mockDb = makeDb({ ...baseDb(),
      purchase_orders: [{ id: 'po-1', company_id: C1, date: '2026-08-01' }, { id: 'po-2', company_id: C1, date: '2026-08-01' }],
      purchase_order_items: [{ id: 'l1', company_id: C1, purchase_order_id: 'po-1' }],
    });
