...
/** Escape a value before interpolating it into email HTML (defense in depth). */
  ...
  const html = `
    <div dir="rtl" style="font-family: 'Segoe UI', Tahoma, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px; background: #f9f9fb; border-radius: 16px;">
      ...
      <div style="text-align: center; margin: 28px 0;">
        <a href="${safeUrl}" style="display: inline-block; padding: 14px 36px; background: #2563eb; color: #fff; text-decoration: none; border-radius: 10px; font-weight: bold; font-size: 15px;">إعادة تعيين كلمة المرور</a>
      </div>
...
export async function sendVerificationEmail(email: string, verifyUrl: string): Promise<boolean> {
  ...
  const html = `
    <div dir="rtl" style="font-family: 'Segoe UI', Tahoma, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px; background: #f9f9fb; border-radius: 16px;">
      ...
      <p style="color: #333; font-size: 15px; line-height: 1.7;">مرحباً،</p>
      <p style="color: #333; font-size: 15px; line-height: 1.7;">اضغط على الرابط أدناه لتأكيد عنوان بريدك الإلكتروني في <strong>AccWeb</strong>. هذا الرابط صالح لمدة <strong>24 ساعة</strong>:</p>
      <div style="text-align: center; margin: 28px 0;">
        <a href="${safeUrl}" style="display: inline-block; padding: 14px 36px; background: #16a34a; color: #fff; text-decoration: none; border-radius: 10px; font-weight: bold; font-size: 15px;">تأكيد البريد الإلكتروني</a>
      </div>
      <p style="color: #666; font-size: 13px; line-height: 1.6;">إذا لم تطلب تغيير بريدك الإلكتروني، يمكنك تجاهل هذه الرسالة.</p>
      <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;" />
      <p style="color: #999; font-size: 11px; text-align: center;">AccWeb © ${new Date().getFullYear()} — نظام محاسبة متكامل</p>
    </div>`;
  return sendEmail(email, 'تأكيد البريد الإلكتروني - AccWeb', html);
}
